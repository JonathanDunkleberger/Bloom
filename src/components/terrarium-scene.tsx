...
export function TerrariumScene({ habits, getStage, isHappy, pct, bouncingId }: TerrariumSceneProps) {
  const allDone = pct >= 1 && habits.length > 0;
  const half = pct >= 0.5;
  const skyTop = allDone ? "#87CEEB" : pct > 0 ? "#A8D8EA" : "#C5D5CB";
  const skyMid = allDone ? "#B5E8C3" : pct > 0 ? "#C2DEBB" : "#C8CCBA";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {half && (
          <>
            <circle cx="345" cy="42" r={allDone ? 42 : 30} fill="url(#sun)">
              <animate
                attributeName="r"
                values={allDone ? "40;46;40" : "28;33;28"}
                dur="4s"
        ...
        {/* Ground layers */}
        <path d="M0 230 Q50 215 100 225 Q150 210 200 222 Q270 212 330 220 Q370 216 400 222 L400 320 L0 320Z" fill="#7BB856" />
        <path d="M0 240 Q80 228 140 238 Q200 225 280 235 Q350 228 400 234 L400 320 L0 320Z" fill="#6DAA48" />
        <path d="M0 255 Q100 245 200 252 Q300 242 400 250 L400 320 L0 320Z" fill="#5F9C3C" />
        <path d="M0 295 Q200 290 400 295 L400 320 L0 320Z" fill="#8B6D42" opacity="0.25" />
        ...
        {Array.from({ length: Math.floor(pct * 18) }).map((_, i) => {
          ...
          const x = 15 + r() * 370;
          const y = 250 + r() * 28;
          const cls = ["#FF6B9D", "#FFB347", "#FF85A2", "#F9D56E", "#C678DD", "#66D9EF", "#FF5E5B", "#D4A5FF", "#98D8C8"];
          const fc = cls[Math.floor(r() * cls.length)];
          const ps = 2 + r() * 2;
          return (
            <g key={`f${i}`}>
